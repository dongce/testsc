...
*** 사용자 코드 작성 <<user_code>>
    *유닛테스트* 더블클릭 >>  *테스트 정보* >> *사용자 코드*
    ...
    파라미터는 *testsc* 의 HOME 경로가 된다. 그 경로아래에서 [[scheme_file][scheme 파일]] 을
    불러온다. 
...
** [[scheme_file][scheme 파일]]작성
   앞서 지정한 폴더에 [[scheme_file][scheme 파일]]을 다음과 같은 순서로 작성한다. 

   1. 필요한 경우  [[scheme_file][scheme 파일]]에서는 공통 정의파일을 포함한다. 테스트 대상에서
   ...
   2. 테스트변수를 define 한다. 
      scheme 의 define 을 이용하여 변수를 선언한다. [[testsc_struct][testsc 구조체]]를 사용하는
      경우 anset 
      CodeScroll 이 정의한 변수인 경우 편리를 위해 같은 이름으로 정의한다.
      CodeScroll 이 정의하지 아니한 값이라도 [[scheme_file][scheme 파일]] 에서 정의하여 사용할 수
      있다. stub 등을 작성할 때 관련 변수를 사용할 수 있다. 
   ...
   3. [[scheme_test_script][scheme 파일 작성]]부분을 참조하여 *TESTCASENO* 에 따라 테스트변수에 값을
      설정한다. 
      
      상세한 내용은 [[scheme_test_script][scheme 파일 작성]]부분을 참조한다
