...
*** 테스트 데이터 조합 가능
    ...
    파라미터에 대하여 개별로 테스트 데이터를 생성해 주는 것으로 보이며 변수들의
...
* 사용방법 
    ...
    :END:
** Codescroll 에서 테스트케이스 생성 
   기존 방식 대로 CodeScroll 메뉴얼을 참고하여 테스트케이스를 생성한다. 
** Codescroll 에서 [[scheme_file][scheme 파일]] 호출 부분을 작성한다. 
...
*** 관련 헤더파일 include <<global_include>>
...
*** 사용자 코드 작성 <<user_code>>
    *유닛테스트* 더블클릭 >>  *테스트 정보* >> *사용자 코드*
    ...
    #+BEGIN_SRC cpp
      testsc_init(CS_TESTCASENO(), "(testsc-require mb/t-9)", "c:/testsc/");
    #+END_SRC

    [[testsc_init]] 를 호출하여 testsc 를 초기화 한다. 첫번째 파라미터는
    CS_TESTCASENO() 으로써 테스트번호를 전달하게 된다. 두번째 파라미터는 실행할
    scheme 구문이다. [[testsc-require]] 를 호출하여 [[scheme_file][scheme 파일]]을 읽어 온다. 마지막
    파라미터는 *testsc* 의 HOME 경로가 된다.  [[testsc-require]] 는 홈 경로아래에서
    [[scheme_file][scheme 파일]] 을 불러온다. 경로를 파라미터로 넘기지 아니할 경우 프로젝트가
    있는 드라이브의 testsc 폴더가 홈이 된다. ( C 드라이브에 프로젝트 폴더가 있을
    경우 c:\testsc\ 폴더, 리눅스의 경우 //testsc//) 
...
**** 호출 전 코드 
     *유닛테스트* 더블클릭 >>  *테스트 정보* >> *테스트 대상 함수* >> *호출 전 코드*
     ...
     CodeScroll 의 테스트데이터과 같이 사용하고자 할 경우 *CS_TESTCASENO()* 를
     이용하여 관련 부분이 실행이 아니되게 할 수 있다. 다음과 같이 실행하며 10
     보다 큰 테스트케이스에 대해서만 [[scheme_file][scheme 파일]]로 부터 값을 읽어 온다. 
...
**** stub 코드 작성 
     테스트대상 함수 내부에 있는 값들은 CodeScroll 이 생성하지만 stub 의 동작을
     제어하기 위한 변수는 생성을 하지 아니한다.  

     stub 에서 사용할 변수 역시 [[scheme_file][scheme 파일]]에서 지정하고 관련 값을 stub을
     구현하는데 사용할 수 있다. 

     테스트전역 코드에 *testsc.h* 파일이 포함되어 있기 때문에 별도의 선언 없이
     [[scheme_file][scheme 파일]] 에서 testsc 관련 함수를 사용할 수 있다. 
    
** [[scheme_file][scheme 파일]] 작성 <<make_scheme_file>>


   [[make_scheme_file][scheme 파일 작성]]을 할 때는 CodeScroll이 필요 없다. CodeScroll에서
   [[test_case][테스트 케이스]]를 닫아 라이센스를 절약할 수 있다.  
   ...
   3. [[scheme_test_script][scheme 파일 작성]]부분을 참조하여 *TESTCASENO* 에 따라  [[test_variable][테스트 변수]]를
      ...
      상세한 내용은 [[scheme_test_script][scheme 파일 작성]] 및 [[testsc 구조체][testsc_struct]]부분을 참조한다
...
*** 빌드 및 [[test_case][테스트 케이스]]를 실행한다. 
    ...
    다음 파일은 [[testcase][테스트케이스]]가 실행하면서 출력하는 것을 저장하는 파일이다. 

    - testsc-stderr.txt : [[testsc_home][testsc 홈]] 폴더 아래 위치한다. [[testcase][테스트케이스]]가
      실행하면서 출력하는 표준에러 출력을 저장한다. 

    - testsc-stdout.txt : [[testsc_home][testsc 홈]] 폴더 아래 위치한다. [[testcase][테스트케이스]]가
      실행하면서 출력하는 표준출력 및 [[testsc_debug]] 출력을 저장한다. 

* scheme 파일 작성 <<scheme_test_script>>
    :PROPERTIES:
    :ATTACH_DIR: u:/orgdir/testsc/
    :END:
** scheme 파일 작성 법
...
*** 테스트 변수를 확인한다. 
    ...
    등을 구현하는데 사용할 수 있다. [[stub_implementation][스텁구현 참조]]
    #+BEGIN_SRC scheme
      (define value1 10)
