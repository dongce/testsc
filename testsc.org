...
*** 사용자 코드 작성 
    *유닛테스트* 더블클릭 >>  *테스트 정보* >> *사용자 코드*
    ...
    파라미터의 경로는 testsc 의 HOME 경로가 된다. 그 경로아래에 불러올
    스크립트가 있어야 한다.  
...
**** stub 코드 작성 
     ...
     CodeScroll 이 생성하는 테스트변수 뿐만 아니라 stub 에서 사용할 변수 역시
     [[scheme_file][scheme 파일]]에서 지정할 수 있다. 
...
** scheme 파일 작성
   ...
   함수별로 [[scheme_file][scheme 파일]]이 존재하며 [[scheme_file][scheme 파일]] 내에서는 테스트케이스 번호에
   따라 테스트 변수에 값을 설정한다. 

   상세한 내용은 [[*scheme 파일 작성][scheme 파일 작성]] 부분을 참조한다
** testsc 확장 모듈 
   C 를 이용하여 확장 할 수 있다. 
*** 구조체 테스트 데이터 생성 
...
*** 일괄 빌드 수행
    ...
    최대한 scheme 파일에서 테스트 데이터를 가져오도록 하면 CodeScroll 이
    재컴파일하는 시간을 줄일 수 있다. 
*** testsc 확장 모듈 
...
*** 비관리 코드 활용. 
    CodeScroll 테스트케이스 생성 및 일괄 빌드 후 비관리 코드 전환을 수행하면
    ScodeScroll 없이도 테스트를 편집할 수 있다. 
**** 일괄 비관리 코드 전환
