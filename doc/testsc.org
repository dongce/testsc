...
** 무엇을 하는 것인가? 
   ...
   프로그래밍을 통한 데이터의 생성이 가능하다. 기본 타입(long, double, char*),
   구조체 및 자료구조(List, Tree 등) 데이터를 생성할 수 있다. 

    #+BEGIN_SRC dot :file image/concept.png :exports results
      digraph {
      rankdir =LR ; 
...
*** 포함 파일 추가
    ...
    [[file:image/include.png][file:image/include.png]]
...
*** 링크 라이브러리 추가 
    ...
    [[file:image/testsclink.png][file:imange/testsclink.png]]
...
** 예제 <<example>>
   ...
   *파일 >> 워크스페이스 바꾸기* 메뉴를 이용하여 예제가 있는 워크스페이스로 전환한다. 
   ...
   [[file:image/chworksspace.PNG][file:image/chworksspace.PNG]]
...
***** 에디터 실행 
     ...
     다음과 같이 실행파일을 선택하라는 다이얼로그가 팝업 한다. 

     =/testsc/home/scheme.exe/= 파일을 선택하고 열기를 수행한다. 

     [[file:image/setlisppath.png][file:image/setlisppathscheme.png]]
     
     
    IDE 가 실행되면 *Settings>>Keyword Set>>Scheme* 을 선택한다. 

    [[file:image/keyword.png][file:image/keyword.png]]

***** 에디터에서 파일 열기 
      파일 메뉴에서 =/testsc/home/ex/t-1.scm= 파일 열기를 수행한다. 
      ...
      #+BEGIN_SRC scheme
        ...
        (testsc-apply-tdata
         1                   ; testcase 시작 옵셋
         (lambda (a b )      ; testcase 번호에서 시작 옵셋값을 뺀 번째 값을 파라미터로 전달한다. 
...
***** 테스트데이터 확인 
****** [[testsc-init]]실행해 보기
       에디터의 아래쪽 [[REPL]] 에서 다음을 입력해 본다. [[testsc_tnum][테스트 번호]]을 242로
       설정하고 [[testsc_home][testsc 홈]] 폴더 아래 =ex/t-1.scm= 파일을 실행한다. 
      
       #+BEGIN_SRC scheme
       (testsc-init 242 ex/t-1)
       #+END_SRC

       [[REPL]] 에서 [[test_variable][테스트 변수]]값을 입력하면 설정된 값을 확인할 수 있다. 


       [[file:image/testsc-init-exam.png][file:image/testsc-init-exam.png]]
      
       
****** [[scheme_file][scheme 파일]] 일부분 실행해 보기 
       [[combi]] 함수 호출 끝 부분에 마우스커서를 위치 시킨 후 위쪽 툴바의 =()= 를
       실행한다. 이 메뉴는 [[S-exp]] 를 [[REPL]]에서 실행한다. 생성되는 [[tests_data][테스트 데이터]]를
       확인할 수 있다. 

       [[file:image/sendtolisp.png][file:image/sendtolisp.png]]
...
** scheme 파일 에디터 
** tutorial 
   [[file:scheme/www.shido.info/lisp/idx_scm_e.html][file:scheme/www.shido.info/lisp/idx_scm_e.html]]
** 15분 만에 scheme 배우기
   #+BEGIN_SRC scheme
