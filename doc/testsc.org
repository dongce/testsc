...
* 기본 사항 
** 무엇을 하는 것인가? 
   동적 시험 시 필요한 [[test_variable][테스트 변수]]를 파일로 부터 읽어 오는 라이브러리이다.
...
*** 커버리지 평가 
    ...
    테스트데이터를 생성한다. CodeScroll에 추가적으로 입력 하는 부분 없이 [[scheme_file][scheme
    파일]]을 수정하여 테스트데이터를 생성할 수 있다. 
...
**** [[scheme_file][scheme 파일]]
     ...
     - 추가적인 [[test_variable][테스트 변수]] 값이 필요할 경우 scheme-case 문을 사용하여 지정할 수
       있다. 
...
\\
      | 용도     | [[test_variable][테스트 변수]]의 정수 값을 가져온다.     |
      | 파라미터 | name → 가져오고자 하는 [[test_variable][테스트 변수]]명 |
      |          | name → 가져오고자 하는 [[test_variable][테스트 변수]]명 |
      | 리턴 값  | [[test_variable][테스트 변수]]의 값                      |
...
**** double testsc_dvalue(const char* name)  <<testsc_dvalue>>
     ...
     #+END_SRC
      - 용도 : [[test_variable][테스트 변수]]의 부동 소수값을 가져온다. 
      - 파라미터 :
        + name → 가져오고자 하는 [[test_variable][테스트 변수]]명
      - 사용 예 :
        #+BEGIN_SRC cpp
...
**** void testsc_init(int testnum, const char *cmd, const char *homepath = 0 )  <<testsc_init>>
     ...
     [[testsc_init]] 를 호출하여 *testsc* 를 초기화 한다. 첫번째 파라미터는
     CS_TESTCASENO() 으로써 [[testsc_tnum][테스트 번호]]를 전달한다. 두번째 파라미터는 실행할
